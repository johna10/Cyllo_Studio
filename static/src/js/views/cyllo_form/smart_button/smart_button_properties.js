...
export class SmartButtonProperties extends Component {
    setup() {
        this.rpc = useService('rpc');
         console.log("weeeeeee",this.props.properties)
    ...
    onWillStart(async () =>{
            await this.findGroupIds(this.props.properties?.attrs?.groups)
                    });

//    onWillStart(async () =>{
//             cyPath: this.props.properties?.attrs?.['cy-xpath'],
//
//            await this.findGroupIds(this.props.properties?.attrs?.groups)
//
//        });
...
//    }
   ...
   updateFieldType(path, field){
    ...
    onDomainRadioClick(event) {;
        this.state.invisible = event.target.checked ? "true" : "false"
    }
//   async addSmartButton() {
//         console.log("asfddddddddddddddddd",this.props.path)
...
//                    addButtonBox: this.props.properties.addButtonBox ? this.props.properties.addButtonBox : null,
//                    path:this.props.path,
//                    new_button:this.props.properties.new_button || null,
//                }
//            })
//           if(response){
...
//            this.env.bus.trigger('CLEAR_MENU');
//
//        }
//        else {
...
//    }
   async updateSmartButton(){
    console.log("asfdddddddddddddddddupda",this.state.cyPath)
        if(this.state.label) {
            ...
            try {
                const response =  await this.rpc("cyllo_studio/update/smart_button", {
                 kwargs: {
                    model: this.props.viewDetails.model,
                    label: this.state.label,
                    icon: this.state.oldicon !== this.state.icon ? this.state.icon : false,
                    invisible: this.state.invisible || '',
                    groups: this.state.group_ids || '',
